...
function TaskForm({ task, projects, onClose }: { task: TaskView | null; projects: ProjectOption[]; onClose: () => void }) {
  ...
  async function submit(data: FormData) {
    ...
    } catch (caughtError) { setError(caughtError instanceof Error ? caughtError.message : "Не удалось сохранить задачу."); } finally { setPending(false); }
  }
  ...
  async function remove() {
    ...
    try {
      ...
      if (!response.ok) { const payload = await response.json().catch(() => null) as { error?: { message?: string } } | null; throw new Error(payload?.error?.message || "Не удалось удалить задачу."); }
      onClose(); router.refresh();
    } catch (caughtError) { setError(caughtError instanceof Error ? caughtError.message : "Не удалось удалить задачу."); } finally { setDeleting(false); }
  }
...
export function TaskBoard({ initialActiveTasks, activeCount, archiveCount, initialActivePageInfo, projects }: { initialActiveTasks: TaskView[]; activeCount: number; archiveCount: number; initialActivePageInfo: PageInfo; projects: ProjectOption[] }) {
  ...
  async function loadTasks(nextTab: TaskTab, cursor?: string) {
    ...
    try {
      ...
      if (!response.ok || !payload?.tasks || !payload.pageInfo) throw new Error(payload?.error?.message || "Не удалось загрузить задачи.");
    ...
    } catch (caughtError) { setError(caughtError instanceof Error ? caughtError.message : "Не удалось загрузить задачи."); } finally { setLoadingTab(null); }
  }
  ...
  async function setTaskStatus(task: TaskView, status: TaskStatus) { setUpdatingId(task.id); setError(null); try { const response = await fetch(`/api/tasks/${task.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status }) }); const payload = await response.json().catch(() => null) as { error?: { message?: string } } | null; if (!response.ok) throw new Error(payload?.error?.message || "Не удалось изменить статус задачи."); router.refresh(); } catch (caughtError) { setError(caughtError instanceof Error ? caughtError.message : "Не удалось изменить статус задачи."); } finally { setUpdatingId(null); } }
