...
export function MobileNavigation() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200/80 bg-white/90 px-3 pb-[max(0.65rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur-xl md:hidden">
      <div className="mx-auto grid max-w-md grid-cols-5 items-end">
        {links.slice(0, 2).map(({ href, label, icon: Icon }) => (
          <Link key={href} href={href} className={`flex min-h-12 flex-col items-center justify-center gap-1 text-[10px] font-medium ${pathname === href ? "text-blue-600" : "text-slate-400"}`}>
        ...
        <Dialog>
          ...
          <DialogContent className="bottom-0 top-auto max-w-none translate-y-0 rounded-b-none rounded-t-[28px] p-6 pb-9 sm:bottom-1/2 sm:max-w-sm sm:-translate-y-1/2 sm:rounded-[28px]">
            ...
            <div className="grid gap-2 pt-2">
              ...
              <p className="px-2 py-2 text-center text-xs leading-5 text-slate-500">Откройте проект, чтобы за 10 секунд добавить расход, платёж или материал.</p>
            </div>
          </DialogContent>
        </Dialog>
