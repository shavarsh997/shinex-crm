...
export const PATCH = withErrorHandling(async (request, context: TaskRouteContext) => {
  ...
  await requireConfirmation(request, user.id, "task-update", taskId);
...
export const DELETE = withErrorHandling(async (request, context: TaskRouteContext) => {
  const user = await requireUser();
  const { taskId } = await context.params;
  await requireConfirmation(request, user.id, "task-delete", taskId);
