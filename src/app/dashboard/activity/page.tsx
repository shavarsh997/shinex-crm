...
export default async function ActivityPage() {
  ...
  const expenses = await prisma.expense.findMany({ where: { deletedAt: null, project: { deletedAt: null, ...(user.role === "ADMIN" ? {} : { OR: [{ userId: user.id }, { members: { some: { userId: user.id, deletedAt: null } } }] }) } }, include: { project: { select: { title: true } } }, orderBy: [{ date: "desc" }, { createdAt: "desc" }], take: 30 });
