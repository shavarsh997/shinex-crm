...
export function ProjectSettingsDialog({ projectId, status, completedAt, frozenAt, canManageProject, canHardDelete, triggerClassName = "size-10 rounded-full bg-white text-slate-600 shadow-sm ring-1 ring-slate-200 hover:bg-slate-50 hover:text-slate-950" }: ProjectSettingsDialogProps) {
  ...
  async function submitConfirmation(flow: "complete" | "delete") {
    ...
    try {
      ...
      const fallback = flow === "delete" ? "Не удалось удалить проект." : "Не удалось завершить проект.";
  ...
  const title = screen === "settings" ? "Управление проектом" : screen === "complete-warning" ? "Завершить проект?" : screen === "delete-warning" ? "Удалить проект безвозвратно?" : isDeleteFlow ? "Подтвердите удаление" : "Подтвердите завершение";
  const description = screen === "settings" ? "Здесь можно управлять статусом проекта. Необратимое удаление доступно только администратору." : screen === "complete-warning" ? "После завершения финансовые данные и настройки проекта станут недоступны для изменений." : screen === "delete-warning" ? "Удалятся проект, расходы, поступления, изменения бюджета, задачи, участники и вся история операций. Восстановить данные нельзя." : "Чтобы исключить случайное действие, вручную введите показанный код.";
  ...
  return <Dialog open={open} onOpenChange={close}><DialogTrigger render={<Button aria-label="Настройки проекта" variant="ghost" size="icon" className={triggerClassName} />}><Settings2 className="size-5" /></DialogTrigger><ResponsiveDialogContent className="p-5 pb-8 sm:p-7"><DialogHeader><DialogTitle className="text-xl tracking-[-0.035em]">{title}</DialogTitle><DialogDescription>{description}</DialogDescription></DialogHeader>
    {screen === "settings" && <div className="mt-5 grid gap-3">
      ...
      {canHardDelete && <section className="rounded-2xl border border-red-200 bg-red-50 p-4 text-red-950"><div className="flex gap-3"><Trash2 className="mt-0.5 size-5 shrink-0 text-red-700" /><div><p className="font-semibold">Удалить проект безвозвратно</p><p className="mt-1 text-sm leading-5 text-red-900/80">Будут удалены все связанные финансовые данные, задачи и история. Действие нельзя отменить.</p></div></div><Button type="button" variant="destructive" className="mt-4 w-full rounded-xl" disabled={pending} onClick={() => { setError(null); setScreen("delete-warning"); }}><Trash2 className="size-4" />Удалить проект</Button></section>}
    ...
    {screen === "delete-warning" && <div className="mt-5"><div className="flex gap-3 rounded-2xl bg-red-50 p-4 text-red-950"><AlertTriangle className="mt-0.5 size-5 shrink-0" /><p className="text-sm leading-6">Проект и все связанные с ним данные будут физически удалены из базы. Восстановление невозможно.</p></div>{error && <p role="alert" className="mt-4 text-sm text-destructive">{error}</p>}<div className="mt-6 flex justify-end gap-3"><Button type="button" variant="outline" className="rounded-xl" onClick={() => setScreen("settings")}>Назад</Button><Button type="button" variant="destructive" className="rounded-xl" disabled={pending} onClick={() => void requestChallenge("delete")}>{pending ? "Готовим…" : "Продолжить"}</Button></div></div>}
