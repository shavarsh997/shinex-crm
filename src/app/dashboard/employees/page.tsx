...
export default async function EmployeesPage() {
  ...
  return <div className="mx-auto max-w-4xl px-5 pb-28 pt-6 sm:px-8 sm:pt-8"><header className="flex items-start justify-between gap-4"><div><p className="text-xs font-medium uppercase tracking-[0.16em] text-slate-400">SHINEX CRM</p><h1 className="mt-1 text-[28px] font-semibold tracking-[-0.05em] text-slate-950">Работники</h1><p className="mt-1 text-sm text-slate-500">Карточки работников, зарплаты и выплаты по проектам.</p></div><span className="grid size-11 place-items-center rounded-2xl bg-blue-50 text-blue-600"><UsersRound className="size-5" /></span></header><EmployeeDirectory employees={view} canCreate={user.role !== "MEMBER"} /></div>;
}
